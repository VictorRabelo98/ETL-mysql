...
CREATE SCHEMA bronze_layer;  -- Dados brutos
CREATE SCHEMA silver_layer;  -- Dados limpos
CREATE SCHEMA gold_layer;    -- Dados analíticos

-- Usar o schema bronze para começar
USE bronze_layer;
...
-- Tabela de clientes (com dados sujos propositalmente)
...
-- ============================================
-- INSERÇÃO DE DADOS BRUTOS (SIMULAÇÃO)
-- ============================================

-- Clientes (com problemas: duplicatas, emails inválidos, datas erradas)
...
('C001', 'Maria Silva Santos', 'maria.silva@email.com', '(11) 98765-4321', '2023-01-15', 'São Paulo', 'SP', 'Brasil', '1985-03-20', NOW()), -- duplicata
...
-- Livros (com problemas: preços negativos, anos inválidos, categorias inconsistentes)
...
-- Pedidos (com problemas: datas futuras, status inconsistentes, valores nulos)
...
('ORD005', 'C005', '2024-03-22', '', 'Cartão de Crédito', 'Cancelado', NULL, '0', NOW()), -- valor vazio
...
-- Avaliações (com problemas: ratings fora da escala, datas inválidas)
...
SELECT 'Clientes carregados:' AS info1, COUNT(*) AS total FROM raw_customers;
SELECT 'Livros carregados:' AS info2, COUNT(*) AS total FROM raw_books;
SELECT 'Pedidos carregados:' AS info3, COUNT(*) AS total FROM raw_orders;
SELECT 'Itens de pedido carregados:' AS info4, COUNT(*) AS total FROM raw_order_items;
SELECT 'Avaliações carregadas:' AS info5, COUNT(*) AS total FROM raw_reviews;